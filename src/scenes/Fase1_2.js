...
export class Fase1_2 extends CenaBase {
    ...
    create(data) {
        ...
        // --- CORREÇÃO: Removido o 'suffix' da criação das animações ---
        this.anims.create({
            key: 'andarMiniBoss',
        ...
        const miniBoss = this.physics.add.sprite(500, 500, 'andarMiniBoss'); // Inicia com a textura base
        ...
        miniBoss.vida = 300;
        miniBoss.tempoAtaque = 0; // Inicializa o tempo de ataque
    ...
    update() {
        ...
        this.inimigos.children.iterate((miniBoss) => {
            ...
            const isAttacking = miniBoss.anims.isPlaying && miniBoss.anims.currentAnim.key === 'ataqueMiniBoss';
            if (isAttacking) {
            ...
            const dist = Phaser.Math.Distance.Between(this.player.x, this.player.y, miniBoss.x, miniBoss.y);
            ...
            if (dist < 80 && this.time.now > miniBoss.tempoAtaque) {
                ...
                miniBoss.setVelocity(0);
                miniBoss.play('andarMiniBoss', false); 
            }
        });
